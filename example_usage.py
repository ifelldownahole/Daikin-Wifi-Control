...
"""
ac = ieatdirt.DaikinController(uart_id=1, tx=17, rx=16, baudrate=2400,
...
"""
checksum = ieatdirt.calculate_checksum(b"F1")
...
"""
packet = ieatdirt.assemble_packet(ieatdirt.DaikinQuery.POWER_MODE_TEMP_FAN.value)
...
"""
parsed_command, parsed_payload = ieatdirt.parse_packet(packet)
...
"""
encoded_temp = ieatdirt.encode_temp(22)
...
"""
decoded_temp = ieatdirt.decode_temp(encoded_temp)
...
"""
ac.turn_on(temp=20, fan=ieatdirt.DaikinFanSpeed.SPEED_3, mode=ieatdirt.DaikinMode.COOL)
...
"""
payload = ac.query_payload(ieatdirt.DaikinQuery.PROTOCOL_VERSION)
print("Protocol version:", ieatdirt.decode_protocol_version(payload))

"""
decode_model(payload)
Decode the model identifier payload (FC) into a string.
"""
payload = ac.query_payload(ieatdirt.DaikinQuery.MODEL)
print("Model:", ieatdirt.decode_model(payload))

"""
decode_room_temp(payload)
Decode the RH room temperature payload (reversed numeric) → float °C.
"""
payload = ac.query_payload(ieatdirt.DaikinQuery.ROOM_TEMP)
print("Room temp:", ieatdirt.decode_room_temp(payload), "°C")

"""
decode_outside_temp(payload)
Decode the Ra outside temperature payload (reversed numeric) → float °C.
"""
payload = ac.query_payload(ieatdirt.DaikinQuery.OUTSIDE_TEMP)
print("Outside temp:", ieatdirt.decode_outside_temp(payload), "°C")

"""
decode_inlet_temp(payload)
Decode the RI inlet temperature payload (reversed numeric) → float °C.
"""
payload = ac.query_payload(ieatdirt.DaikinQuery.INLET_TEMP)
print("Inlet temp:", ieatdirt.decode_inlet_temp(payload), "°C")

"""
decode_humidity(payload)
Decode the Re humidity payload (reversed integer) → int %.
"""
payload = ac.query_payload(ieatdirt.DaikinQuery.HUMIDITY)
print("Humidity:", ieatdirt.decode_humidity(payload), "%")

"""
decode_fan_rpm(payload)
Decode the RL fan RPM payload (reversed decimal × 10) → int RPM.
"""
payload = ac.query_payload(ieatdirt.DaikinQuery.FAN_RPM)
print("Fan RPM:", ieatdirt.decode_fan_rpm(payload))

"""
decode_compressor_rpm(payload)
Decode the Rd compressor RPM payload (reversed integer) → int RPM.
"""
payload = ac.query_payload(ieatdirt.DaikinQuery.COMPRESSOR_RPM)
print("Compressor RPM:", ieatdirt.decode_compressor_rpm(payload))

"""
decode_louver_angle(payload)
Decode the RN louver angle payload (reversed integer) → int degrees.
"""
payload = ac.query_payload(ieatdirt.DaikinQuery.LOUVER_ANGLE)
print("Louver angle:", ieatdirt.decode_louver_angle(payload))

"""
decode_power_consumption(payload)
Decode the FM energy consumption payload (reversed hex) → int Wh.
"""
payload = ac.query_payload(ieatdirt.DaikinQuery.POWER_CONSUMPTION)
print("Energy used:", ieatdirt.decode_power_consumption(payload), "Wh")
...
"""
payload = ac.query_payload(ieatdirt.DaikinQuery.TEMPS_ALT)
temps = ieatdirt.decode_temps_alt(payload)
...
"""
payload = ac.query_payload(ieatdirt.DaikinQuery.POWER_MODE_TEMP_FAN)
f1 = ieatdirt.decode_f1_response(payload)
print("F1 decoded:", f1)
...
"""
payload = ac.query_payload(ieatdirt.DaikinQuery.SWING_HUMIDITY)
swing = ieatdirt.decode_swing_humidity(payload)
...
"""
payload = ac.query_payload(ieatdirt.DaikinQuery.POWERFUL_QUIET_LED)
pql = ieatdirt.decode_powerful_quiet_led(payload)
...
"""
payload = ac.query_payload(ieatdirt.DaikinQuery.DEMAND_ECO)
de = ieatdirt.decode_demand_eco(payload)
...
"""
swing_payload = ieatdirt.make_swing_payload(vertical=True, horizontal=False)
ac.send_command(ieatdirt.DaikinSetter.SWING_HUMIDITY, payload=swing_payload)
...
"""
pq_payload = ieatdirt.make_powerful_quiet_led_payload(
    powerful=True, quiet=False, comfort=False,
    streamer=False, sensor=False, led=2   # 2 = bright
)
ac.send_command(ieatdirt.DaikinSetter.POWERFUL_QUIET_LED, payload=pq_payload)
...
"""
de_payload = ieatdirt.make_demand_eco_payload(demand=75, eco=True)
ac.send_command(ieatdirt.DaikinSetter.DEMAND_ECO, payload=de_payload)
...
"""
raw_features = ac.query_payload(ieatdirt.DaikinQuery.FEATURES)
print("Features raw:", raw_features)

raw_fk = ac.query_payload(ieatdirt.DaikinQuery.FEATURE_BITS)
